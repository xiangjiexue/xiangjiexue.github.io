...
`r badge_devel("xiangjiexue/npfixedcompR", "blue")`
`r badge_custom("on CRAN", "no", "red")`

**Note that this package will be freezed. The npfixedcomp2 is under ongoing development.**
...
`r badge_last_commit("xiangjiexue/npfixedcomp2")` 
`r badge_devel("xiangjiexue/npfixedcomp2", "blue")`
`r badge_custom("on CRAN", "no", "red")`


This is package is to replace npfixedcompR in the future. The npfixedcompR package
is written in R, it is relatively slow and has some issue. This package is written
in C++ using the Eigen library.

The homepage of this package can be found [here](https://xiangjiexue.github.io/npfixedcomp2) and
the source code of this package can be found [here](https://github.com/xiangjiexue/npfixedcomp2).

For now, nptll, npnormll, npnormllw, npnormcvm, npnormcvmw, npnormad npnormadw and npnormcll have been implemented and it is uniformly faster than the corresponding function in npfixedcompR.

TO DO list:

 - investigate the grid points for distance-based methods. For the computation using density (all the likelihood-based methods implemented), we can obtain a set of grid points that contains all the potential support points. This is not the case for distance-based methods which uses cumulative distribution functions. An example of this is shown in npnormad(w). The NPMDE is not obtained since the boundary of the grid points are in the support point, which shows that the method need points outside of this support space.
 
 - It may be interesting to investigate chaining in Eigen, and see how this can be applied in the package.
...
* microbenchmark `r badge_cran_release("microbenchmark", "orange")` 
`r badge_dependencies("microbenchmark")` `r badge_cran_checks("microbenchmark")`
